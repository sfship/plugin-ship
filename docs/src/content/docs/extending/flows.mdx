...
import SchemaModeline from '../../../components/SchemaModeline.astro';
...
The plugin publishes a JSON Schema for flow definitions, and ejected flows start with a `yaml-language-server` modeline pointing at it — pinned to the plugin version that ejected the file. Editors with a YAML language server (in VS Code, the [YAML extension](https://marketplace.visualstudio.com/items?itemName=redhat.vscode-yaml)) use it for validation, autocompletion, and hover documentation.

For flows you create by hand, add the modeline as the first line. This shows the latest release — if you're on an older plugin version (shown by `sf plugins`), substitute yours:

<SchemaModeline schema="flow" />

The schema validates flow structure — step shape, param definitions, conditions — but not task-specific `params`, since those depend on which task each step runs.
