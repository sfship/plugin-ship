...
The plugin publishes a JSON Schema for `ship.yml`, and `sf ship project init` writes a `yaml-language-server` modeline as the file's first line — pinned to the plugin version that scaffolded it. Editors with a YAML language server (in VS Code, the [YAML extension](https://marketplace.visualstudio.com/items?itemName=redhat.vscode-yaml)) use it for validation, autocompletion, and hover documentation for every field on this page.

<SchemaModeline schema="ship" />
